...
CCaptionPanel::CCaptionPanel()
{
	...
	m_hFont = CreateFont(20, 0, 0, 0, 0, 0, 0, 0, GB2312_CHARSET, 0, 0, 0, 0, _T("微软雅黑"));
}
...
int CCaptionPanel::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	{
		m_btnMin.Create(_T("MinButton"), WS_CHILD | WS_VISIBLE, CRect(rcClient.Width() - 100, 0, rcClient.Width() - 50, 30), this, IDC_MINBTN);
		m_btnMin.SetImage(IDB_MINBTN);
	}

	if (m_btnCls.m_hWnd == NULL)
	{
		m_btnCls.Create(_T("ClsButton"), WS_CHILD | WS_VISIBLE, CRect(rcClient.Width() - 50, 0, rcClient.Width(), 30), this, IDC_CLSBTN);
...
void CCaptionPanel::OnPaint()
{
	...
	if (pMainFrame)
	{
		...
		HICON hIcon = (HICON)::LoadIcon(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDI_ICON26));
		if (hIcon)
		{
			DrawIconEx(dc.m_hDC, 10, 3, hIcon, 26, 26, 0, 0, DI_NORMAL);
		}
		CString strCaption;
